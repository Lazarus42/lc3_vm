...
// This handles the memory of the VM. In total, it supports 128KB.
...
// We initialized the registers and stored them in an array

// Creating the opcodes
enum 
{
OP_BR = 0,
OP_ADD,
            ...
            case OP_LD:
                {
                    ...
                    reg[r0] = mem_read(reg[R_PC] + pc_offest);
                    update_flags(r0);
                }
                break;
            ...
            case OP_ST:
                {
                    uint16_t r0 = (instr >> 9) & 0x7;
                    uint16_t pc_offest = sign_extend(instr & 0x1FF, 9);
                    mem_write(reg[R_PC] + pc_offest, reg[r0]);
                }
                break;
            ...
            case OP_STI:
                {
                    ...
                    uint16_t pc_offest = sign_extend(instr & 0x1FF, 9);
                    mem_write(mem_read(reg[R_PC] + pc_offest), reg[r1]);
                }
                break;
